...
## Surety fluency notes (deep-research pass, July 2026)

Verdict from the research: the pipeline shape and the handoff boundary match how carriers and intake vendors (Federato, Cytora, Indico) frame this, and our tier facts match RLI's live site. The risk was vocabulary: surety is credit, not insurance, and P&C terms are an outsider tell. Fixed in the demo: "ACORD forms" became contractor questionnaires and bond request forms (surety barely uses ACORD; 501/502 only), "risk classification" became triage (the insurance phrase means rating class codes), "bind and issue" became issue and execute (bonds are executed by an attorney-in-fact under a power of attorney).

RLI specifics to get right in the room: FirstStep is one word, Next Step is two, both registered marks; current limits are $500K and $1.5M (older docs say $350K/$1M and are stale). Contract intake is My Contract Bond App (FirstStep, e-signed GIAs) plus agent-to-underwriter email; RLink is the transactional platform, not a contract channel. FirstStep requires no financial statements, so never demo financials-chasing at that tier. RLI runs three surety lines (contract, commercial, transactional); do not lump notary or license-and-permit under contract.

Homework facts for the pitch: 30 consecutive years of underwriting profit (2025 combined ratio 83.6; surety segment 80.3). FY2025 surety GPW: commercial $61.5M, transactional $54.1M, contract $47.1M and shrinking, which is the growth-without-headcount argument. RLI's 10-K calls transactional surety "highly automated," so the pitch is extending a proven playbook up-market, not introducing automation. COO Jen Klobnak (Q4 2025 call) named summarizing submission information as an AI use case so people "spend more time using their brains instead of doing administrative tasks": nearly our pitch in her words. ESOP culture since 1975; underwriters in more than a dozen locations; frame AI as arming the local underwriter.

Fluency tokens: surety credit, bonding line, single/aggregate limits, GIA with spousal signatures at small tiers, clearance, prequalification, the three Cs (character, capacity, capital), WIP profit fade, submission-to-quote ratio, graduated programs / graduation between tiers.

## Reconciliation: conflicts are just flagged fields

When two documents in a package disagree (email says $1,850,000, contract says $1,900,000), the resolution path is the existing review queue, not a new mechanism. Three tiers. First, deterministic precedence: signed contract beats bond request form beats questionnaire beats email prose; latest financials win; exact beats rounded. That order is plain code, and like the SOP register it is elicited from the carrier's review team, not invented. Second, the model proposes: a reconciliation call outputs a value, a confidence, and citations to both sources; immaterial fields that clear the gate auto-resolve with an audit note recording the conflict. Third, the gate routes the rest to a human: low confidence, no applicable rule, or any material field (money, bond terms) regardless of confidence, the same way UW-03 surfaces judgment calls instead of making them.

The loop that retires the escalation: every human pick is a labeled precedence decision. When the queue shows the same pattern repeatedly (brokers round, contracts are exact, the underwriter always picks the contract), promote it to a tier-1 deterministic rule and that conflict class stops reaching the queue. Escalation is the mechanism by which the system earns the right to stop escalating: the autonomy-is-earned thesis applied to one more capability.
