...
from core.extract import MODEL, extract_with_meta
...
def run(save: bool = False, model: str | None = None) -> dict[str, float]:
    ...
    cases = _load_val()
    if not cases:
        print("No val cases found. Run: python -m core.generate --n 12")
        return {}
...
def test_extraction_accuracy():
    ...
    if not _load_val():
        pytest.skip("no synthetic val data (run core.generate)")
    acc = run()
